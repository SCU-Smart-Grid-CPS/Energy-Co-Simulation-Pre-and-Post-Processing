...
numEPlusCalibrationRows = 2305 #2305 - 1  # -1 because first row is column headers
...
while i < ns:
    if "output=" in sys.argv[i]: #must go before filenames because will include .csv in string
        ...
        elif "legacy" in sys.argv[i]: wholesaleType = 'l'
...
if 'l' in wholesaleType:
    ...
    wholesale= np.genfromtxt(pfile, skip_header=0, max_rows=dataend, delimiter=',',usecols=0)
...
for f in files:
    ...
    if 'l' in wholesaleType: # Legacy - older files
        ...
        for w in range(datastart,dataend):
            priceStep = (wholesale[w]*pmultiplier/1000 + poffset)*data['Electricity:Facility [J](TimeStep)'].iloc[w]*2.77778e-7
...
if "None" not in outFile and "none" not in outFile:
    ...
    rows = [files,totalPrice,avgDailyCost,totHeatElec,totCoolElec,avgDailyEnergy,meanDiff100,meanComfBand,pctTimeComf90,pctTimeComf80]
